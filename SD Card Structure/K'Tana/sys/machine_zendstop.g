...
; This file sets the Z position of the Z end stop at the bottom of the printer.  It is used to determine the
; height of the build volume. If this value is incorrect, you either have a nozzle that drags on the bed or 
; is too far from the bed that the filament won't stick to the surface.  It is the single most important file 
; to get right so do spend some time verifying the Z position after making changes here.

; Instructions
; 1. Home All, this will set the Z-height to the current Z-value listed below
; 2. Run G1 X200 Y200 Z20 F1500, this will move your nozzle to the approximate center
; 3. Manually jog the bed up using the <Z-10 to <Z-0.1 using smaller steps as you get close to the nozzle
; 4. Have bed touching the nozzle, do not leave a gap.  If you can't reach the nozzle, estimate how far the
;    bed is from the nozzle and add that value to the Z value in the G92 command below and start over.
; 5. Record your Z height number as shown in the DWC Machine Status Head Position Window
; 6. Calculate the new Z value for the G92 command below:
;
...
G92 Z375.5 ; With glass bed and K'tana
